/* ============================================================
   Jisson Alex & Catherine Therese Thomas — Wedding Invitation
...
(function () {
  ...
  /* ---------------------------------------------------------
     8 · Countdown — 4 Jan 2027, 11:00 AM IST
     --------------------------------------------------------- */
  (function countdown() {
    /* Wedding: Monday 4 January 2027 at 11:00 AM IST (UTC+5:30) */
    var target = new Date('2027-01-04T11:00:00+05:30').getTime();
...
(function () {
  ...
  var SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyE566ADAnbhESwiKssTJPX4CWbg7bedmjCCfc5jIDgFAOKOrzmiZGPv3vXuPA139FGRg/exec';
  /* ──────────────────────────────────────────────────────────────────────── */
  ...
  function validate() {
    ...
    var phone = document.getElementById('rsvpPhone');
    var phoneOk = phone && /^[\d\s\+\-\(\)]{6,16}$/.test(phone.value.trim());
    markInvalid(phone, !phoneOk);
    showErr('errPhone', !phoneOk);
    if (!phoneOk) ok = false;

    var guests = document.getElementById('rsvpGuests');
    var guestsOk = guests && guests.value !== '';
    markInvalid(guests, !guestsOk);
    showErr('errGuests', !guestsOk);
    if (!guestsOk) ok = false;

    var attendingChecked = form.querySelector('input[name="attending"]:checked');
    showErr('errAttending', !attendingChecked);
    if (!attendingChecked) ok = false;
  ...
  form.addEventListener('submit', function (e) {
    ...
    /* Show success after 2.5 s — plenty of time for the POST to land */
    setTimeout(function () {
      setLoading(false);
      form.hidden        = true;
